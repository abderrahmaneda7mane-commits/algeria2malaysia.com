...
export default function APUCoursesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]" dir="rtl">
      ...
      {/* ── Hero ── */}
      <div ref={reveal} className="section-reveal relative overflow-hidden bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 text-white pt-20 pb-20 px-4">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-96 h-96 bg-blue-400/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-blue-300/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
        ...
        <div className="relative max-w-5xl mx-auto">
          <button onClick={() => go("universities")} className="flex items-center gap-2 text-blue-200 hover:text-white transition-colors mb-8 group text-sm">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 bg-blue-500/30 border border-blue-400/30 rounded-full px-3 py-1 text-xs font-semibold text-blue-100 mb-2">
                جامعة تقنية · Top Tech University
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-white leading-tight">
                جامعة APU — Asia Pacific University
              </h1>
              <p className="text-blue-200 text-sm mt-1.5">Asia Pacific University of Technology & Innovation — Technology Park, KL</p>
              {!loading && !error && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 bg-gray-50 focus:bg-white transition-all"
              />
            </div>
            <div className="relative">
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 appearance-none bg-gray-50 focus:bg-white transition-all"
              >
                <option value="all">كل المدد الدراسية</option>
                {allDurations.map(d => (
                  <option key={d} value={d}>{d} {Number(d) === 1 ? "سنة" : "سنوات"}</option>
                ))}
              </select>
            </div>
            <div className="relative">
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 appearance-none bg-gray-50 focus:bg-white transition-all text-left"
              >
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {courses.map(course => (
                <div
                  key={course.id}
                  className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-blue-200 hover:-translate-y-0.5 transition-all duration-200 group"
                >
                  <h3 className="font-bold text-gray-900 text-sm leading-snug mb-1 group-hover:text-blue-700 transition-colors">{course.name}</h3>
                  <p className="text-blue-600 text-xs font-semibold mb-4">{course.universities?.[0]?.name ?? "APU"}</p>
                    ...
                    <span className="flex items-center gap-1.5 text-xs">
                      ...
                      {course.price == null || course.price === 0 ? (
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold text-blue-600">€{toEur(course.price)}</span>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-blue-50 hover:border-blue-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                >
