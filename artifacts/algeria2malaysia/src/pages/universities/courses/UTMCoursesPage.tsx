...
export default function UTMCoursesPage() {
  ...
  } = useCourses(UTM_UNIVERSITY_ID, PRICE_RANGES);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]" dir="rtl">
      ...
      {/* ── Hero ── */}
      <div ref={reveal} className="section-reveal relative overflow-hidden bg-gradient-to-br from-sky-900 via-sky-800 to-sky-700 text-white pt-20 pb-20 px-4">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-96 h-96 bg-sky-400/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-sky-300/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
        ...
        <div className="relative max-w-5xl mx-auto">
          <button onClick={() => go("universities")} className="flex items-center gap-2 text-sky-200 hover:text-white transition-colors mb-8 group text-sm">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center flex-shrink-0 p-2.5 shadow-xl">
              <img src="/logos/utm.png" alt="UTM" className="max-w-full max-h-full object-contain" />
            </div>
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 bg-sky-500/30 border border-sky-400/30 rounded-full px-3 py-1 text-xs font-semibold text-sky-100 mb-2">
                جامعة حكومية · Top Ranking
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-white leading-tight">
                جامعة تكنولوجيا ماليزيا — UTM
              </h1>
              <p className="text-sky-200 text-sm mt-1.5">Universiti Teknologi Malaysia — Johor Bahru & KL</p>
              {!loading && !error && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="ابحث باسم التخصص..." value={search} onChange={e => setSearch(e.target.value)}
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400/30 focus:border-sky-400 bg-gray-50 focus:bg-white transition-all" />
            </div>
            <div className="relative">
              <Clock size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
              <select value={durationFilter} onChange={e => setDurationFilter(e.target.value)}
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400/30 focus:border-sky-400 appearance-none bg-gray-50 focus:bg-white transition-all">
            ...
            <div className="relative">
              ...
              <select dir="ltr" value={priceRangeIdx} onChange={e => setPriceRangeIdx(Number(e.target.value))}
                className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400/30 focus:border-sky-400 appearance-none bg-gray-50 focus:bg-white transition-all text-left">
        ...
        {!loading && !error && courses.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {courses.map(course => (
                <div key={course.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-sky-200 hover:-translate-y-0.5 transition-all duration-200 group">
                  <h3 className="font-bold text-gray-900 text-sm leading-snug mb-1 group-hover:text-sky-700 transition-colors">{course.name}</h3>
                  <p className="text-sky-600 text-xs font-semibold mb-4">{course.universities?.[0]?.name ?? "UTM"}</p>
                  <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-500">
                    ...
                    <span className="flex items-center gap-1.5 text-xs">
                      <DollarSign size={12} className="text-sky-400 flex-shrink-0" />
                      {course.price == null || course.price === 0 ? (
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold text-sky-600">€{toEur(course.price)}</span>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-sky-50 hover:border-sky-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                ...
                {pageNumbers.map((num, i) => num === "..." ? (
                  ...
                  <button key={num} onClick={() => setPage(num as number)}
                    className={`w-10 h-10 rounded-xl text-sm font-bold transition-all border ${page === num ? "bg-sky-600 text-white border-sky-600 shadow-md" : "bg-white text-gray-600 border-gray-200 hover:bg-sky-50 hover:border-sky-300"}`}>
                    {num}
                  </button>
                ))}
