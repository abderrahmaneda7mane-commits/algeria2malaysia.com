...
const PRICE_RANGES = [
  ...
  { label: "RM 10,000 – 30,000", min: 10000, max: 30000 },
  { label: "RM 30,000 – 50,000", min: 30000, max: 50000 },
  { label: "RM +50,000", min: 50000, max: Infinity },
];
    ...
    <div className="min-h-screen bg-[#f8fafc]" dir="rtl">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-purple-800 to-purple-700 text-white pt-20 pb-20 px-4">
        ...
        <div className="relative max-w-5xl mx-auto">
          <button onClick={() => go("universities")} className="flex items-center gap-2 text-purple-200 hover:text-white transition-colors mb-8 group text-sm">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 bg-purple-500/30 border border-purple-400/30 rounded-full px-3 py-1 text-xs font-semibold text-purple-100 mb-2">
                جامعة خاصة · Top Ranked
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-white leading-tight">
                جامعة تايلور — Taylor's University
              </h1>
              <p className="text-purple-200 text-sm mt-1.5">Taylor's University Lakeside Campus — Subang Jaya</p>
              {!loading && !error && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="ابحث باسم التخصص..." value={search} onChange={e => setSearch(e.target.value)}
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400/30 focus:border-purple-400 bg-gray-50 focus:bg-white transition-all" />
            </div>
            <div className="relative">
              <Clock size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
              <select value={durationFilter} onChange={e => setDurationFilter(e.target.value)}
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400/30 focus:border-purple-400 appearance-none bg-gray-50 focus:bg-white transition-all">
            ...
            <div className="relative">
              ...
              <select dir="ltr" value={priceRangeIdx} onChange={e => setPriceRangeIdx(Number(e.target.value))}
                className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400/30 focus:border-purple-400 appearance-none bg-gray-50 focus:bg-white transition-all text-left">
        ...
        {!loading && !error && courses.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {courses.map(course => (
                <div key={course.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-purple-200 hover:-translate-y-0.5 transition-all duration-200 group">
                  <h3 className="font-bold text-gray-900 text-sm leading-snug mb-1 group-hover:text-purple-700 transition-colors">{course.name}</h3>
                  <p className="text-purple-600 text-xs font-semibold mb-4">{course.universities?.[0]?.name ?? "Taylor's University"}</p>
                  <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-500">
                    ...
                    <span className="flex items-center gap-1.5 text-xs">
                      <DollarSign size={12} className="text-purple-400 flex-shrink-0" />
                      {course.price == null || course.price === 0 ? (
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold text-purple-600">€{toEur(course.price)}</span>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-purple-50 hover:border-purple-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                ...
                {pageNumbers.map((num, i) => num === "..." ? (
                  ...
                  <button key={num} onClick={() => setPage(num as number)}
                    className={`w-10 h-10 rounded-xl text-sm font-bold transition-all border ${page === num ? "bg-purple-600 text-white border-purple-600 shadow-md" : "bg-white text-gray-600 border-gray-200 hover:bg-purple-50 hover:border-purple-300"}`}>
                    {num}
                  </button>
                ))}
